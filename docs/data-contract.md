...
`collect --analyse-access-log` streams the selected log and normal rotations, filters rows to the exact recorder window, then counts requests, methods and HTTP response statuses. RunCloud date suffixes are treated as rotation boundaries: rotations that can contain the window are included, while older rotations and a current log known to begin after the window are skipped. Unfamiliar and numbered rotation schemes remain conservatively included.
